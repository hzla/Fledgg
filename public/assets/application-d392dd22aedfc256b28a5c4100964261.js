...
(function() {
  var Conversation, ready;

  Conversation = {
    init: function() {
      $('body').on('click', '.conversation', this.selectConvo);
      $('body').on('click', '.reply-svg', this.reply);
      $('body').on('ajax:success', '.convo-link', this.displayConvo);
      $('body').on('ajax:success', '#new_message', this.showMessages);
      $('body').on('ajax:beforeSend', '#new_message', this.checkMessage);
      $('body').on('ajax:success', '.trash-link', this.trashConversation);
      $('body').on('click', '.bar-section', this.changeTab);
      $('body').on('ajax:success', '#meetings-link', this.showMeetings);
    ...
    showMeetings: function(event, data) {
      ...
      return $('#conversations').prepend($('.meeting-link.unaccepted'));
    },
    changeTab: function() {
    ...
    startPage: function() {
      if ($('#send-to').length > 0) {
        ...
        if ($('body.mobile').length < 1) {
          $('.convo-link').first().click();
        }
      }
      return $('#conversations').prepend($('.conversation.unread'));
    },
    showMessages: function() {
      ...
      return $('#new-message-container').hide();
    },
    selectConvo: function() {
    ...
    reply: function() {
      ...
      $('#message_conversation_id').val($(this).attr('id'));
      name = $('.conversation.active').find('.convo-name').text();
      return $('#message-recipient').text(name);
    },
...
(function() {
  ...
  Feedback = {
    init: function() {
      $('body').on('submit', '#feedback-form', this.thankUser);
      $('body').on('click', '#feedback', this.showForm);
      $('#content-left, #user-top-bar, #green-bar, #status-bar').click(this.hideFeedback);
      return Feedback.on = false;
    },
    hideFeedback: function() {
      console.log($(this));
      if ($(this).parents('#feedback-container').length > 0) {
        return false;
      }
      if (Feedback.on) {
    ...
    showForm: function() {
      $('#feedback-container input, #feedback-container textarea').animate({
        ...
        return Feedback.on = true;
      });
      return $('#feedback-container').show().animate({
        height: "340px"
      }, 1000);
    },
    thankUser: function() {
      ...
      return setTimeout(function() {
        return $('#send-feedback').val('Submit');
      }, 2000);
    }
  };
...
(function() {
  ...
  Meeting = {
    init: function() {
      ...
      $('body').on('click', '#content-right', this.hideMeetingForm);
      $('body').on('click', '.meeting-request', this.showMeetingForm);
      $('body').on('ajax:success', '.meeting-link', this.showMeeting);
      $('body').on('ajax:success', '#new_meeting', this.notifyMeeting);
      $('body').on('submit', '#new_meeting', this.showSending);
      $('body').on('click', '.current-meeting-btn', this.hideMeeting);
      $('body').on('ajax:success', 'a', this.checkMeetings);
      $('body').on('click', '.star-rating-container .star', this.highlightStar);
      $('body').on('mouseleave', '.star-rating-container .star', this.unhighlightStar);
      $('body').on('click', '.star-rating-container .star', this.rate);
      $('body').on('ajax:success', '#rating-form', this.thankUser);
      $('body').on('click', '#meeting-cancel', this.hideMeetingForm);
      $('.xdsoft_scroller_box').unbind();
      Meeting.canRate = true;
      return this.checkMeetings();
    },
    initDateTimePicker: function() {
      ...
      return $('#meeting_start_time').click(function() {
        setTimeout(function() {
          return $('.xdsoft_time_variant').parent().scrollTo($('.xdsoft_time:not(.xdsoft_disabled)').last());
        }, 100);
        return setTimeout(function() {
          var a;
          console.log("hi");
          a = $('.xdsoft_time_variant').parent().scrollTop();
          return $('.xdsoft_time_variant').parent().scrollTop(a - 125);
        }, 700);
      });
    },
    dateLogic: function(dateTime) {
    ...
    dateLogicScroll: function(dateTime) {
      if (dateTime.getDate() === (new Date().getDate())) {
        console.log("this");
        return this.setOptions({
          minTime: 0
    ...
    showSending: function() {
      return $('#send-meeting').val('Sending...');
    },
    thankUser: function() {
      return $('.message-body').html("<h4>Thank You for Rating!</h4>");
    },
    rate: function() {
      var rating;
      rating = $(this).attr('id');
      $('#rating-value').val(rating);
      $(this).nextAll().find('polygon').css('fill', 'gray');
      Meeting.rated = true;
      Meeting.canRate = false;
      return setTimeout(function() {
        return Meeting.canRate = true;
      }, 300);
    },
    unhighlightStar: function() {
      return Meeting.rated = false;
    },
    highlightStar: function() {
      $('.star').find('polygon').css('fill', '#2ecc71');
      $(this).nextAll().find('polygon').css('fill', 'gray');
      return Meeting.rated = false;
    },
    checkMeetings: function() {
      if ($('#current-meeting').length < 1) {
        return $.get('/meetings/check', function(data) {
          if ($('#current-meeting').length < 1) {
    ...
    hideMeeting: function() {
      return $('#current-meeting').remove();
    },
    notifyMeeting: function(event, data) {
      $('body').prepend(data);
      $(this)[0].reset();
    ...
    showMeeting: function(event, data) {
      ...
      return $('.message').addClass('animated fadeIn');
    },
    hideMeetingForm: function() {
      $('#send-meeting').val('REQUEST MEETING');
      if (Meeting.open = true) {
        $('#meeting-modal-container').hide();
      }
      Meeting.open = false;
      return true;
    },
    showMeetingForm: function() {
      var btn, id;
      id = $(this).attr('id');
      btn = $(this);
      setTimeout(function() {
        ...
        if (btn.hasClass('result-meeting-request')) {
          return $('#meeting_recipient').val(id);
        }
      }, 10);
      return $('#meeting-modal-container, #meeting-modal-content').swipe({
...
(function() {
  ...
  Search = {
    init: function() {
      $('body').on('click', '.search-svg', this.submitSearch);
      $('body').on('keypress', '#people-search-form input', this.submitOnEnter);
      $('body').on('click', '.follow-link', this.showFollowed);
      return $('.result').hoverIntent(this.displayResultInfo, this.doNothing);
    },
    showFollowed: function() {
      if ($(this).hasClass("unfollow")) {
        $(this).find('div').text('+ FOLLOW');
        return $(this).removeClass().addClass('follow-link follow');
      } else {
        $(this).find('div').text('UNFOLLOW');
        return $(this).removeClass().addClass('follow-link unfollow');
      }
    },
    doNothing: function() {},
    displayResultInfo: function(event) {
      var id;
      console.log;
      id = $(this).attr('id').slice(2);
      return $.get("/users/" + id + "/result_info", function(data) {
        $('#content-right').html(data);
        return $('#profile-content').addClass('animated fadeIn');
      });
    },
    ...
    submitSearch: function() {
      return $('#people-search-form').submit();
    }
  };
...
(function() {
  ...
  Status = {
    init: function() {
      $('body').on('ajax:success', '#new_status', this.addStatus);
      $('body').on('click', '.like-svg', this.likeStatus);
      $('body').on('ajax:success', '#new_comment', this.addComment);
      $('body').on('ajax:beforeSend', '#new_comment', this.checkBody);
      this.scrollMeetings();
      $('.like-link').on('ajax:success', this.like);
      $('body').on('ajax:success', '.status-delete-link', this.deleteStatus);
      return $('body').on('ajax:success', '.comment-delete-link', this.deleteComment);
    },
    deleteStatus: function() {
      return $(this).parents('.status').removeClass('animated fadeIn').addClass('animated fadeOut').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function() {
        return $(this).remove();
      });
    },
    deleteComment: function() {
      return $(this).parents('.comment').removeClass('animated fadeIn').addClass('animated fadeOut').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function() {
        return $(this).remove();
      });
    },
    scrollMeetings: function() {
      return $(window).scroll(function() {
        ...
        return right.css('top', "" + position + "px");
      });
    },
    like: function(event, data) {
      var like, newCount;
      like = $(this).children('.like');
      newCount = parseInt(like.next().text()) + 1;
      if (data.dont_like === false) {
        like.next().text(newCount);
        return $(this).children('.like-text').text('Unlike');
      } else {
        like.next().text(newCount - 2);
        return $(this).children('.like-text').text('Like this');
      }
    },
    checkBody: function() {
      var body;
      body = $(this).find('#comment_body').val();
      if (body === "") {
        $('#comment_body').css('border', '1px solid red');
        return false;
      }
    },
    addStatus: function(event, data) {
      $('#statuses').prepend(data);
      $('.status').first().addClass('animated fadeIn');
      return $(this)[0].reset();
    },
    likeStatus: function() {
      return $(this).parents('.status').find('.like').click();
    },
    addComment: function(event, data) {
      var count, new_count;
      $(this).parents('.comment').before(data);
      $(this).parents('.comment').prev().addClass('animated fadeIn');
      count = $(this).parents('.status').find('.comment_count');
...
(function() {
  ...
  TopNav = {
    init: function() {
      $('body').on('touchend', '.mobile-menu-icon', this.openNav);
      $('body').on('touchend', '.top-nav-content a', this.closeNav);
      return $('#content-left, #content-right, #conent-left-small, #content-right-large').click(this.closeNav);
    },
    openNav: function() {
      ...
      return TopNav.state = "open";
    },
    closeNav: function() {
      return $('.top-nav-content').removeClass('animated fadeOutLeftBig').addClass('animated fadeOutLeftBig');
    }
  };
...
(function() {
  var UserProfile, ready;

  UserProfile = {
    init: function() {
      $('body').on('click', '.edit-svg.edit-skill', this.showEditField);
      $('body').on('click', '.edit-svg:not(.edit-skill)', this.editField);
      $('body').on('ajax:success', '#skill-form, #needed-skill-form', this.addSkill);
      $('body').on('click', '.skill', this.deleteSkill);
      $('body').on('ajax:success', '.edit_user', this.saveSettings);
      $('body').on('click', '.skill-input', this.addSkillFromBox);
      $('.best_in_place').bind('ajax:success', this.checkValues);
      $('body').on('ajax:success', 'a, form', this.linkifyLinks);
      return this.linkifyLinks();
    },
    addSkillFromBox: function() {
      var skill;
      skill = $(this).text();
      $('.skill-name').val(skill);
      return $('#needed-skill-form').submit();
    },
    linkifyLinks: function() {
      return $('.linkify').each(function() {
        var linkifiedText;
        linkifiedText = UserProfile.linkify($(this).text());
        return $(this).html(linkifiedText);
      });
    },
    ...
    editField: function() {
      return $(this).parent().next().children().first().click();
    },
    saveSettings: function() {
      $('#save-user').val('Saved!');
      return setTimeout(function() {
        return $('#save-user').val('Save');
      }, 2000);
    },
    ...
    showEditField: function() {
      return $(this).parents('.field-header').next().toggle();
    },
    addSkill: function(event, data) {
      var className, form, skill, skillList;
      $(this)[0].reset();
      skill = data.name;
      if ($('.onboarding.skills').length < 1) {
        skillList = $(this).parents('.field-row').next();
      } else {
        skillList = $('#skill-row');
      }
      if ($(this).attr('id') === "skill-form") {
        className = "have";
      } else {
        className = "needed";
      }
      if (skillList.find("#s-" + data.id).length < 1) {
        ...
        return skillList.find("#s-" + data.id).addClass('animated fadeIn');
      } else {
        form = $(this);
